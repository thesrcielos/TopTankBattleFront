    ...
    useEffect(() => {
        ...
        class MainScene extends Phaser.Scene {
            ...
            create() {
                ...
                this.players = {};
                this.fortress1HpBarBg = this.add.rectangle(48, 416, fortressBarWidth, fortressBarHeight, 0x000000);
                this.fortress1HpBar = this.add.rectangle(48, 416, fortressBarWidth, fortressBarHeight, 0xff0000);
                this.fortress2HpBarBg = this.add.rectangle(1936, 416, fortressBarWidth, fortressBarHeight, 0x000000);
                this.fortress2HpBar = this.add.rectangle(1936, 416, fortressBarWidth, fortressBarHeight, 0xff0000);
                this.hpBarBg = this.add.rectangle(20, 20, barWidth, barHeight, 0x000000).setOrigin(0, 0).setScrollFactor(0);
                // Barra roja
                this.hpBar = this.add.rectangle(20, 20, barWidth, barHeight, 0xff0000).setOrigin(0, 0).setScrollFactor(0);
                for (const playerId in gameState?.players) {
                    const player = gameState.players[playerId];
                    if (!player || !player.position)
                        continue;
                    const sprite = this.physics.add.image(player.position.x, player.position.y, player.team1 ? 'tanque' : 'tanque2')
            ...
            updatePlayers() {
                ...
                for (const id in players) {
                    ...
                    if (sprite.x !== data.x ||
                        ...
                        const step = 0.2;
                        sprite.rotation = Phaser.Math.Angle.RotateTo(sprite.rotation, data.angle || 0, step);
                    }
                }
            }
            spawnBullets() {
                ...
                for (let id in bullets) {
                    ...
                    const bullet = this.otherBullets.create(data.position.x, data.position.y, data.team1 ? "bala" : "bala2");
            ...
            notifyPlayerMoves() {
                ...
                let now = Date.now();
                if ((now - this.lastSent) > 50 && (x !== this.lastPosition.x ||
            ...
            checkGameOver() {
                ...
                const gameOverText = this.add.text(this.cameras.main.width / 2, this.cameras.main.height / 2, message, {
                    fontSize: '68px',
                    color: color,
            ...
            fireMissile(pointer) {
                if (this.lastFired > this.time.now || !this.alive)
                    return;
